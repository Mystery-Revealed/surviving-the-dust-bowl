...
// → approve names → watch live status, class accuracy BY FAMILY, and each
// student's chosen PATH → download the PDF → End Session (deletes everything
// from memory). Surviving the Dust Bowl is branching variant solo, so students
// start on their own as soon as they're approved — no pairing needed. The path
// column is information, never a grade — both stay and go score equally.
...
const FAMILY_LABEL = { owner: 'Farm-Owner', tenant: 'Tenant', town: 'Town' };
const FAMILY_ORDER = ['owner', 'tenant', 'town'];
const PATH_LABEL = { stay: 'Stayed', go: 'Went west' };

function pathText(s) {
  if (!s.path) return s.status === 'completed' || s.status === 'in_progress' ? '—' : '';
  return s.nation ? `${FAMILY_LABEL[s.nation] || s.nation} · ${PATH_LABEL[s.path]}` : PATH_LABEL[s.path];
}
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Surviving the Dust Bowl · Farm-Owner, Tenant, or Town · TEKS 7.7E, 7.9A, 7.9B</p>
  ...
  const familyCls = FAMILY_ORDER.map((f) => [f, roster?.classAccuracy?.[f] || { count: 0, average: 0 }]);
  const completed = students.filter((s) => s.status === 'completed').length;
  const inProgress = students.filter((s) => s.status === 'in_progress').length;
  const stayCount = students.filter((s) => s.status === 'completed' && s.path === 'stay').length;
  const goCount = students.filter((s) => s.status === 'completed' && s.path === 'go').length;
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Surviving the Dust Bowl · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student begins their family’s story right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <p className="muted">{inProgress} in progress · {completed} completed{completed > 0 ? ` (${stayCount} stayed, ${goCount} went west)` : ''}</p>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Student</th><th>Family</th><th>Path</th><th>Status</th><th>Accuracy</th><th></th></tr>
            </thead>
            <tbody>
              {students.length === 0 && (
                <tr><td colSpan={6} className="muted">Nobody has joined yet. Share the class code!</td></tr>
              )}
              {students.map((s) => (
                <tr key={s.id}>
                  <td>
                  ...
                  <td>{FAMILY_LABEL[s.nation] || '—'}</td>
                  <td>{s.path ? <span className={`path-pill ${s.path}`}>{PATH_LABEL[s.path]}</span> : <span className="muted">—</span>}</td>
      ...
      <div className="card">
        <h2>Class accuracy by family</h2>
        <div className="class-summary">
          {familyCls.map(([f, cls]) => (
            <div key={f} className="summary-stat">
              <div className="big-number">{cls.count ? `${cls.average}%` : '—'}</div>
              <div className="muted">{FAMILY_LABEL[f]} · {cls.count} completed</div>
            </div>
          ))}
        </div>
        {completed > 0 && (
          <p className="muted">
            Class split: <b>{stayCount}</b> stayed · <b>{goCount}</b> went west — a nice discussion
            starter: real Panhandle families split the same way.
          </p>
        )}
      ...
      <footer className="app-footer">7th Grade Texas History · Surviving the Dust Bowl · TEKS 7.7E, 7.9A, 7.9B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [138, 90, 43]; // sun-baked wheat brown

  doc.setFontSize(16);
  doc.text('Surviving the Dust Bowl — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  const rows = roster.students.map((s) => [
    s.name,
    FAMILY_LABEL[s.nation] || '—',
    s.path ? PATH_LABEL[s.path] : '—',
  ...
  autoTable(doc, {
    startY: 30,
    head: [['Student', 'Family', 'Path', 'Status', 'Accuracy']],
    body: rows,
    styles: { fontSize: 9 },
  ...
  autoTable(doc, {
    ...
    head: [['Class accuracy by family', '']],
    body: familyCls.flatMap(([f, cls]) => [
      [`${FAMILY_LABEL[f]} — completed`, String(cls.count)],
      [`${FAMILY_LABEL[f]} — average accuracy`, cls.count ? `${cls.average}%` : '—'],
    ]),
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Surviving the Dust Bowl · TEKS 7.7E, 7.9A, 7.9B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`dust-bowl_${roster.joinCode}_${date}.pdf`);
}
